...
class DatabaseOrdenCompra{
    companion object {
        fun insertarOrden(ordenCompra: OrdenCompra){
            "http://192.168.100.28:1337/Orden".httpPost(listOf("total" to ordenCompra.cedulaComprador, "fecha" to ordenCompra.sector, "costoEntrega" to ordenCompra.idProducto))
        ...
        fun insertarOrdenDetalles(ordenDetalles: OrdenDetalles){
            "http://192.168.100.28:1337/DetalleOrden".httpPost(listOf("fechaEnvio" to ordenDetalles.fechaEnvio, "precio" to ordenDetalles.precio, "idProducto" to ordenDetalles.idProducto))
                    .responseString { request, _, result ->
                        Log.d("http-ejemplo", request.toString())
                    }
        }
        ...
        fun getOrdenesList(): ArrayList<OrdenCompra> {
            ...
            val (request, response, result) = "http://192.168.100.28:1337/Orden".httpGet().responseString()
